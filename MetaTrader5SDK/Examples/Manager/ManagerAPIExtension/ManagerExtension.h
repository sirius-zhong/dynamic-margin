...
public:
   ...
   void              Shutdown(void);
   //--- connect, disconnect
   bool              Connect(const CMTStr &server,const uint64_t login,const CMTStr &password);
   ...
   MTAPIRES          GetOrdersTotal(const CMTStr &group_name,uint32_t &total)    { return GetTotal(group_name,total,true); }
   MTAPIRES          GetPositionsTotal(const CMTStr &group_name,uint32_t &total) { return GetTotal(group_name,total,false); }
   MTAPIRES          GetUsersTotal(const CMTStr &group_name,uint32_t &total);
private:
   ...
   MTAPIRES          GetTotal(const CMTStr &group_name,uint32_t &total,const bool is_order);
  };
//+------------------------------------------------------------------+
