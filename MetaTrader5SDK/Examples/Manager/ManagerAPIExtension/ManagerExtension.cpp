...
//---    
   ...
   uint32_t version=0;
//--- Manage Factory init
   if((retcode=m_factory.Initialize(nullptr))!=MT_RET_OK)
     {
...
//+------------------------------------------------------------------+
bool CManagerExtension::Connect(const CMTStr &server,const uint64_t login,const CMTStr &password)
  {
...
//| Get total                                                        |
//+------------------------------------------------------------------+
MTAPIRES CManagerExtension::GetTotal(const CMTStr &group_name,uint32_t &total,const bool is_order)
  {
   MTAPIRES retcode=MT_RET_ERROR;
//--- check params
   if(group_name.Empty())
...
//+------------------------------------------------------------------+
MTAPIRES CManagerExtension::GetUsersTotal(const CMTStr &group_name,uint32_t &total)
  {
   MTAPIRES retcode=MT_RET_ERROR;
//--- check params   
   if(group_name.Empty())
