         ...
         try
           {
            ...
            if((m_request=m_manager.RequestCreate())==null)
              {
               m_manager.LoggerOutString(EnMTLogCode.MTLogErr,"Dealer: creating request object failed");
            ...
            if((m_confirm=m_manager.DealerConfirmCreate())==null)
              {
               m_manager.LoggerOutString(EnMTLogCode.MTLogErr,"Dealer: creating confirmation object failed");
            ...
            if((res=m_request_sink.Initialize(m_manager,m_event_request))!=MTRetCode.MT_RET_OK)
              {
               m_manager.LoggerOutString(EnMTLogCode.MTLogErr,"Dealer: creating request sink failed");
      ...
      public bool Start(string server,UInt64 login,string password)
        {
         ...
         if(server==null || login==0 || password==null)
           {
            m_manager.LoggerOutString(EnMTLogCode.MTLogErr,"Dealer: starting failed with bad arguments");
            return(false);
           }
