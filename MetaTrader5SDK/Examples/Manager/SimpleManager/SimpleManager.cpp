...
//|                                                                  |
//+------------------------------------------------------------------+
class CBookSink : public IMTBookSink
  {
   virtual void      OnBook(const MTBook& book) override
     {
      wprintf(L"=== BOOK START ===\nSymbol   : %s\n",book.symbol);
      wprintf(L"Time    : %I64d\n",book.datetime);
      wprintf(L"TimeMsc : %I64d\n",book.datetime_msc);
      wprintf(L"Flags   : %I64u\n",book.flags);
      for(uint32_t i=0;i<book.items_total;i++)
         wprintf(L"%s - %I64d - %.5lf\n",
                 book.items[i].type==MTBookItem::ItemSell ? L"SELL" : L"BUY",
                 book.items[i].volume,
                 book.items[i].price);
      wprintf(L"=== BOOK END ===\n");
     }
   ...
   if(user)
     {
      ...
      //--- change password
      res=manager->UserPasswordChange(IMTUser::USER_PASS_INVESTOR,user->Login(),L"ewq3212!");
      //--- advanced security
      ...
      if(mail)
        {
         ...
         mail->To(user->Login());
         mail->Body(body,uint32_t(wcslen(body)+1)*sizeof(*body));
...
//--- custom command
   MTLogRecord record     ={ LOG_FLAGS_NONE,MTLogOK,MTLogTypeAPI,_time64(nullptr),L"SimpleManager",L"custom command" };
   LPVOID      outdata    =NULL;
