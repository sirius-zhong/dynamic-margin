...
//| Dealer starting                                                  |
//+------------------------------------------------------------------+
bool CDealer::Start(LPCWSTR server,uint64_t login,LPCWSTR password)
  {
   DWORD id=0;
//--- check manager
...
//--- check arguments
   if(!server || login==0 || !password)
     {
      m_manager->LoggerOutString(MTLogErr,L"Dealer: starting failed with bad arguments");
      return(false);
     }
...
//--- start dealing thread
   ...
   m_connected=FALSE;
//--- start thread
   if((m_thread_dealer=(HANDLE)_beginthreadex(NULL,STACK_SIZE_COMMON,DealerWrapper,this,STACK_SIZE_PARAM_IS_A_RESERVATION,(uint32_t*)&id))==NULL)
     {
      m_manager->LoggerOutString(MTLogErr,L"Dealer: starting dealer thread failed");
      return(false);
     }
//--- done
   return(true);
...
//--- parent window
   m_parent=parent;
//--- initialize factory
   if((res=m_factory.Initialize(nullptr))!=MT_RET_OK)
     {
...
//--- create request object
   if((m_request=m_manager->RequestCreate())==NULL)
     {
      m_manager->LoggerOutString(MTLogErr,L"Dealer: creating request object failed");
...
//--- create confirmation object
   if((m_confirm=m_manager->DealerConfirmCreate())==NULL)
     {
      m_manager->LoggerOutString(MTLogErr,L"Dealer: creating confirmation object failed");
   ...
   if(m_event_request==NULL)
     {
      ...
      m_manager->LoggerOutString(MTLogErr,message);
      AfxMessageBox(message);
      return(false);
     }
//--- create requests event
...
//+------------------------------------------------------------------+
uint32_t __stdcall CDealer::DealerWrapper(LPVOID param)
  {
//--- dealing thread
