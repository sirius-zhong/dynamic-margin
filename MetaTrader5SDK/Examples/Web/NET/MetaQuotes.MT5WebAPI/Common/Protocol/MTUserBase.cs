...
namespace MetaQuotes.MT5WebAPI.Common.Protocol
  ...
  class MTUserBase : MTAPIBase
    ...
    public MTRetCode Add(MTUser user,out MTUser newUser)
      ...
      byte[] answer;
      //--- send request
      Dictionary<string,string> paramsUser = new Dictionary<string,string>();
      //---
    ...
    public MTRetCode Delete(ulong login)
      {
      //--- send request
      Dictionary<string,string> data = new Dictionary<string,string> { { MTProtocolConsts.WEB_PARAM_LOGIN,login.ToString() } };
      //--- get answer
      byte[] answer;
      //---
    ...
    public MTRetCode UserLogins(string group,out List<ulong> logins)
      ...
      logins = null;
      //--- send request
      Dictionary<string,string> data = new Dictionary<string,string> { { MTProtocolConsts.WEB_PARAM_GROUP,group } };
      //--- get answer
      byte[] answer;
      //---
    ...
    private static MTRetCode ParseUser(string command,string answer,out MTUser newUser)
      ...
      //---
      MTUserAnswer userAnswer = new MTUserAnswer();
      //--- get param
      int posEnd = -1;
      MTAnswerParam param;
    ...
    private static MTRetCode ParseAccount(string command,string answer,out MTAccount account)
      ...
      //---
      MTAccountAnswer accountAnswer = new MTAccountAnswer();
      //--- get param
      int posEnd = -1;
      MTAnswerParam param;
      while((param = MTParseProtocol.GetNextParam(answer,ref pos,ref posEnd)) != null)
    ...
    private static MTRetCode ParseLogins(string command,string answer,out List<ulong> logins)
      ...
      //---
      MTLoginsAnswer loginsAnswer = new MTLoginsAnswer();
      //--- get param
      int posEnd = -1;
      MTAnswerParam param;
      while((param = MTParseProtocol.GetNextParam(answer,ref pos,ref posEnd)) != null)
  ...
  class MTUserAnswer : MTBaseAnswerJson
    ...
    public MTUser GetFromJson()
      ...
      try
        ...
        serializer.RegisterConverters(new List<JavaScriptConverter> { new MT5UserConverter() });
        return serializer.Deserialize<MTUser>(ConfigJson);
        }
      catch(Exception e)
        {
  ...
  /// </summary>
  class MT5UserConverter : JavaScriptConverter
    {
    private static readonly Type[] m_SupportedTypes = new[] { typeof(MTUser) };
  ...
  class MTAccountAnswer : MTBaseAnswerJson
    ...
    public MTAccount GetFromJson()
      ...
      try
        {
        JavaScriptSerializer serializer = new JavaScriptSerializer { MaxJsonLength = MAX_LENGHT_JSON };
        serializer.RegisterConverters(new List<JavaScriptConverter> { new MTAccountConverter() });
        return serializer.Deserialize<MTAccount>(ConfigJson);
        }
      catch(Exception e)
        {
  ...
  /// </summary>
  class MTAccountConverter : JavaScriptConverter
    {
    private static readonly Type[] m_SupportedTypes = new[] { typeof(MTAccount) };
    public override IEnumerable<Type> SupportedTypes { get { return m_SupportedTypes; } }
    /// <summary>
    /// Deserialize from MT server json
    /// </summary>
  ...
  class MTLoginsAnswer : MTBaseAnswerJson
    ...
    public List<ulong> GetFromJson()
      ...
      try
        {
        JavaScriptSerializer serializer = new JavaScriptSerializer { MaxJsonLength = MAX_LENGHT_JSON };
        serializer.RegisterConverters(new List<JavaScriptConverter> { new MTLoginsConverter() });
        return serializer.Deserialize<List<ulong>>(ConfigJson);
        }
      catch(Exception e)
        {
  ...
  /// </summary>
  class MTLoginsConverter : JavaScriptConverter
    {
    private static readonly Type[] m_SupportedTypes = new[] { typeof(ulong) };
    public override IEnumerable<Type> SupportedTypes { get { return m_SupportedTypes; } }
    /// <summary>
    /// Deserialize from MT server json
    /// </summary>
