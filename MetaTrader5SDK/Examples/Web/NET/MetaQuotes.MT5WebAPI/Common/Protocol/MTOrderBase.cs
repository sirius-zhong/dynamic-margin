...
namespace MetaQuotes.MT5WebAPI.Common.Protocol
  ...
  class MTOrderBase : MTAPIBase
    ...
    public MTRetCode OrderGet(ulong tick,out MTOrder order)
      ...
      order = null;
      //--- send request
      Dictionary<string,string> data = new Dictionary<string,string> { { MTProtocolConsts.WEB_PARAM_TICKET,tick.ToString() } };
      //--- get answer
      byte[] answer;
      //--- send request
...
public MTRetCode OrderGetPage(ulong login,uint offset,uint total,out List<MTOrder> orders)
    ...
    private static MTRetCode ParseOrderTotal(string answer,out uint total)
      ...
      //---
      MTOrderTotalAnswer groupAnswer = new MTOrderTotalAnswer();
      //--- get param
      int posEnd = -1;
      MTAnswerParam param;
      while((param = MTParseProtocol.GetNextParam(answer,ref pos,ref posEnd)) != null)
  ...
  class MTOrderAnswer : MTBaseAnswerJson
    ...
    public MTOrder GetFromJson()
      ...
      try
        ...
        serializer.RegisterConverters(new List<JavaScriptConverter> { new MTOrderConverter() });
        return serializer.Deserialize<MTOrder>(ConfigJson);
        }
      catch(Exception e)
        {
  ...
  class MTOrderPageAnswer : MTBaseAnswerJson
    ...
    public List<MTOrder> GetFromJson()
      ...
      try
        {
        JavaScriptSerializer serializer = new JavaScriptSerializer {MaxJsonLength = MAX_LENGHT_JSON};
        serializer.RegisterConverters(new List<JavaScriptConverter> { new MTOrderPageConverter() });
        return serializer.Deserialize<List<MTOrder>>(ConfigJson);
        }
      catch(Exception e)
        {
  ...
  /// </summary>
  class MTOrderPageConverter : JavaScriptConverter
    {
    private static readonly Type[] m_SupportedTypes = new[] { typeof(MTOrder) };
  ...
  /// </summary>
  class MTOrderConverter : JavaScriptConverter
    {
    private static readonly Type[] m_SupportedTypes = new[] { typeof(MTOrder) };
    public override IEnumerable<Type> SupportedTypes { get { return m_SupportedTypes; } }
    /// <summary>
    ...
    public static object ParseOrder(IDictionary<string,object> dictionary)
      {
      //---
      MTOrder obj = new MTOrder();
      //---
